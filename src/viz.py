...
def show3Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False): # blue, orange
	...
	Args
	channels: 66x1 vector. The pose to plot.
	...
	"""
	nJoints = 22
	assert channels == nJoints * 3, "channels should have 66 entries"
	vals = np.reshape(channels, nJoints, -1)
...
def show2Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
	...
	Args
	channels: 44x1 vector. The pose to plot.
	...
	"""
	nJoints = 22
	assert channels == nJoints * 2, "channels should have 44 entries"
	vals = np.reshape(channels, nJoints, -1)

...
def animation_plot(animations, filename=None, ignore_root=True, interval=33.33):
    ...
    for ai in range(len(animations)):
        ...
        # joints, rootposs_x, rootposs_z, rootdir_x, rootdir_z, rootheight_c = anim[:,:93], anim[:,-9], anim[:,-8], anim[:,-7], anim[:,-6], anim[:,-5]
        # joints = anim[:, :93]
        joints = anim
        ...
        if not ignore_root:
            for i in range(len(joints)):
                ...
                joints[i, :, 0] = joints[i, :, 0] + rootposs_x[i]
                joints[i, :, 2] = joints[i, :, 2] + rootposs_z[i]
    ...
    parents = np.array(
        [-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 15, 13, 17, 18, 19, 20, 21, 22, 13, 24, 25, 26, 27, 28,
         29])

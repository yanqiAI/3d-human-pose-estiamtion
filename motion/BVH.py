...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    # if order == 'zxy':
    #     rotations_ = np.zeros(rotations.shape)
    #     rotations_[:,:,0:1] = rotations[:,:,0:1]
    #     rotations_[:,:,1:2] = rotations[:,:,2:3]
    #     rotations_[:,:,2:3] = rotations[:,:,1:2]
    #     order = 'zyx'
    #     rotations = Quaternions.from_euler(np.radians(rotations_), order=order, world=world)
    # else:
            ...
            for j in range(anim.shape[1]):
                ...
                if positions or j == 0:
                    ...
                    f.write("%f %f %f %f %f %f " % (
                        ...
                        # root_poss[j,0],                  root_poss[j,1],                  root_poss[j,2],
